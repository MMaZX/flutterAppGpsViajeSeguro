...
class CuidadorPacientePage extends ConsumerStatefulWidget {
  const CuidadorPacientePage({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _CuidadorPacientePageState();
}

class _CuidadorPacientePageState extends ConsumerState<CuidadorPacientePage> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: FutureCustomWidget(
            future: PacienteController(context, ref).fetchCuidador(),
            widgetBuilder: (context, snapshot) {
              List<DataFormModelPage> list = snapshot.data;
              if (list.isEmpty) {
                return const EmptyWidget(
                    "No se encontraron cuidadores asociados. Notifica a tu familiar para que te asocie con un cuidador.");
              }
